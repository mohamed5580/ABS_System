    ...
    public partial class Voucher : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataGridView1.Rows.Add(txtDetails.Text, Convert.ToDouble(txtAmount.Text), txtNotes.Text);
                    double k = GrandTotal();
                    k = Math.Round(k, 2);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    // Insert into Voucher
                    string cb = "INSERT INTO Voucher(Id, VoucherNo, Date, Name, Details, GrandTotal, Code) VALUES (@d1, @d2, @d3, @d4, @d4, @d5, @d7,@d8)";
                    using (SqlCommand cmd = new SqlCommand(cb, con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@d8", txtAccountCode.Text);
                        cmd.ExecuteNonQuery();
                    }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    // Update Voucher
                    string cb = "UPDATE Voucher SET VoucherNo = @d2, Date = @d3, Name = @d4, Details = @d5, GrandTotal = @d7 ,Code=@d8 WHERE ID = @d1";
                    using (SqlCommand cmd = new SqlCommand(cb, con))
                    {
                        cmd.Parameters.AddWithValue("@d1", voucherId);
                        cmd.Parameters.AddWithValue("@d2", txtVoucherNo.Text);
                ...
                LogFunc(lblUser.Text, st);
                LedgerUpdate(dtpDate.Value.Date, txtName.Text, 0, -Convert.ToDecimal(txtGrandTotal.Text), "", txtVoucherNo.Text, "سند قبض / صرف");
                btnUpdate.Enabled = false;
        ...
        public static void LedgerSave(DateTime a, string b, string c, string d, decimal e, double f, string g, string h)
        {
            using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
            {
                con.Open();
...
/*            char keyChar = e.KeyChar;
        ...
        private void Button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(DataAccessLayer.Con()))
                {
                    ...
                    // Insert into Voucher
                    string cb = "INSERT INTO Voucher(Id, VoucherNo, Date, Name, Details, GrandTotal,Code) VALUES (@d1, @d2, @d3, @d4, @d5, @d7, @d8)";
                    using (SqlCommand cmd = new SqlCommand(cb, con))
                    {
                        cmd.Parameters.AddWithValue("@d1", voucherId);
                        cmd.Parameters.AddWithValue("@d2", txtVoucherNo.Text);
                    ...
                    using (SqlCommand cmd = new SqlCommand(cb1, con))
                    {
                        foreach (DataGridViewRow row in DataGridView1.Rows)
                        {
                            if (!row.IsNewRow)
                            {
                                ...
                                cmd.Parameters.Clear();
                                cmd.Parameters.AddWithValue("@d1", voucherId);
                                cmd.Parameters.AddWithValue("@d2", particularsStr);
                                cmd.Parameters.AddWithValue("@d3", amount);
                                cmd.Parameters.AddWithValue("@d4", note ?? (object)DBNull.Value);
                ...
                string st = "added the new voucher having voucher no. '" + txtVoucherNo.Text + "'";
                LogFunc(lblUser.Text, st);
                LedgerSave(dtpDate.Value.Date, txtName.Text, txtVoucherNo.Text, "سند قبض / صرف", 0, -grandTotal, "", "");
                MessageBox.Show("تم الحفظ بنجاح", "سندات الصرف", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            AccountsScreen frm = new AccountsScreen();
